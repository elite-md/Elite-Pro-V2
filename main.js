...
try{
		...
		if (update.connection == "connecting" || update.receivedPendingNotifications == "false") {
			console.log(color(`\n🌿Connecting to your device...`, 'yellow'))
		}
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} Whatsapp Bot: ELITE-PRO-V2`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: Elite-MD `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @Chinedu-MD `,'magenta'))
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = ` @${xeonName.split("@")[0]}, have been *promoted* as *admin* 😱`
   XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
const xeondate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let xeonName = num
xeonbody = `𝗢𝗼𝗽𝘀‼️ @${xeonName.split("@")[0]}, you have been *demoted* as *admin* 😬`
XeonBotInc.sendMessage(anu.id,
 { text: xeonbody,
 contextInfo:{
 mentionedJid:[num],
...
ppgroup = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60'
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./ElitePro.js")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
